...
# 直接指定基础路径
vizwiz_data_base_path = '/root/autodl-tmp/VizWiz2024-VQA-AnswerTherapy-main/dataset'

# 指定 VizWiz 训练集图像目录和标注文件路径
vizwiz_datatrain_image_dir = '/root/autodl-tmp/VizWiz2024-VQA-AnswerTherapy-main/dataset/train'
vizwiz_datatrain_annotation_path = '/root/autodl-tmp/VizWiz2024-VQA-AnswerTherapy-main/dataset/Annotations/VizWiz_merged.json'

# 指定 VQA 训练集图像目录和标注文件路径
vqa_datatrain_image_dir = '/root/autodl-tmp/VizWiz2024-VQA-AnswerTherapy-main/dataset/train2017'
vqa_datatrain_annotation_path = '/root/autodl-tmp/VizWiz2024-VQA-AnswerTherapy-main/dataset/Annotations/VQA_merged.json'

# 指定评估集图像目录和标注文件路径
eval_image_dir = '/root/autodl-tmp/VizWiz2024-VQA-AnswerTherapy-main/dataset/val'
eval_annotation_path = '/root/autodl-tmp/VizWiz2024-VQA-AnswerTherapy-main/dataset/Annotations/VizWiz_val.json'
...
def get_model():
    ...
    print(f"Number of trainable parameters: {len(optimizer_params)}")
    ...
    # 修改 get_model 函数中的 new_forward 部分
    def new_forward(*args, **kwargs):
        kwargs['output_hidden_states'] = True
        outputs = original_forward(*args, **kwargs)
    
        # 获取最后一层的隐藏状态
        last_hidden_states = outputs.hidden_states[-1]  # [batch_size, seq_length, hidden_size]
        B, L, H = last_hidden_states.shape
        
        # 使用平均池化而不是重塑为方形特征图
        # 对序列维度进行池化
        pooled_features = last_hidden_states.mean(dim=1)  # [B, H]
        
        # 创建多个特征视图
        feature1 = F.linear(pooled_features, torch.randn(H, H).to(device))
        feature3 = F.linear(pooled_features, torch.randn(H, H).to(device))
        feature5 = F.linear(pooled_features, torch.randn(H, H).to(device))
        
        # 特征融合
        fused_feature = torch.cat([feature1, feature3, feature5], dim=1)
        
        # 添加一个线性层来匹配分类器的输入维度
        projection = nn.Linear(3*H, H).to(device)
        final_feature = projection(fused_feature)
        
        # 更新输出
        outputs.logits = model.classifier(final_feature)
...
def train_model(model, dataset, num_epochs=50):
    ...
    optimizer = torch.optim.AdamW(optimizer_params, lr=5e-5)
    model.train()
    losses = []
...
def visualize_prediction(image, question: str, answer_texts: List[str], save_img=False, fixed_width=600):
    ...
    if save_img:
        output_image_path = f'{question}.jpg'
...
if __name__ == "__main__":
    ...
    my_model = get_model()
    train_model(my_model, dataset, 100)
