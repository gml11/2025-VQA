...
def inference_vilt(input_image, input_text, threshold=0.01, print_preds=True,finetune_folder=None):
    # print("Inference finetune.....")
    # fintune_processor = ViltProcessor.from_pretrained(finetune_folder)
    if finetune_folder is None:
        raise ValueError("inference_vilt 函数调用时未传递 finetune_folder 参数，请确保在调用函数时传入有效的模型路径。")
    try:
        fintune_processor = ViltProcessor.from_pretrained(finetune_folder)
    except Exception as e:
        print(f"加载本地 ViltProcessor 时出错: {e}")
        raise
    model = ViltForQuestionAnswering.from_pretrained(finetune_folder)
    # prepare inputs
    encoding = fintune_processor(input_image, input_text, return_tensors="pt")
    ...
    dynamic_thresh = 0.4 * max_prob
    # results = [model.config.id2label[c.item()] for c, p in zip(classes.squeeze(), probs.squeeze()) if
    #            p >= dynamic_thresh]
    results = []
    for c, p in zip(classes.squeeze(), probs.squeeze()):
        # print(f"Class: {model.config.id2label[c.item()]} | Probability: {p.item()}")
        if p >= dynamic_thresh:
            print(model.config.id2label[c.item()])
            results.append(model.config.id2label[c.item()])
    if not results:
        try:
